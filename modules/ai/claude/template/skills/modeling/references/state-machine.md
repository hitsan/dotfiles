...
Writeツールを使用して設計成果物をファイルに保存する。

**出力先:** `docs/design/機能名-modeling.md`
...
| 状態 | 説明 |
|------|------|
[Step 2で抽出した状態リストを表形式で]

### イベント
| イベント | 説明 |
|----------|------|
[Step 2で抽出したイベントリストを表形式で]

## 状態遷移表
[Step 3で定義した状態遷移表]

## 実装時の考慮事項
[Step 6で整理した実装ガイドライン（重要なポイントのみ）]
```

**対話での出力:**
設計ファイルを保存した後、概要のみを表示:
```
✓ 設計完了

機能: [機能名]
ファイル: docs/design/[機能名]-modeling.md
