...
選択した手法に応じた設計ドキュメントを.md形式で作成・保存する。

**データフロー設計の場合:**
- ファイル名: `[機能名]-dataflow.md`
- 内容:
  1. 設計概要
  2. データ構造定義（入力・中間・出力）
  3. データフロー図（Mermaid形式）
...
**複合設計の場合:**
- 両方のドキュメントを作成
- 統合ガイド（状態とデータフローの対応関係）を追加セクションとして含める

### Step 5: 設計成果物の出力

**初回設計時:**
- 設計の要約をプロンプトに出力する
- 主要な構造、重要な決定事項、考慮事項を簡潔に説明
- 保存したファイルパスを明示

**変更・更新時（ユーザーとの対話で設計を修正した場合）:**
- .mdファイルを更新
- 変更差分をプロンプトに出力
- 何を変更したか、なぜ変更したかを説明
- 更新した箇所を明示
...
- **Mermaid形式を使用**: データフロー図と状態遷移図はMermaid形式で作成する
- **.md形式で保存**: 設計成果物は必ず.mdファイルとして保存する
