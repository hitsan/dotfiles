...
description: 対話形式でステートマシンを設計する専門家。手動起動（/state-machine）で使用。新機能や仕様検討時に、状態・イベント・遷移を段階的に明確化し、ASCII図と遷移表で設計図を作成する。実装は行わず、設計のみで完了する。
---
...
設計パターンや詳細な例については、`references/state_machine_patterns.md` を参照すること。
